...
class TaskCore:
    ...
    def _job_entry(name_id, task_path, load_info):
        ...
        if not osp.exists(osp.join(confpath, 'deployed')):
            ...
            rtn = subprocess.Popen(
                    [load_info['product']['deploy']], shell=True,
    ...
    def register(self, name_id, load_info):
        #
        rtn = self.scheduler.add_job(
                TaskCore._job_entry, 
                trigger=load_info['ASPScheduler']['type'],
                args=None, kwargs={
                            'name_id': name_id,
