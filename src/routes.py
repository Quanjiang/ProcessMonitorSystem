...
taskobj = task_core.TaskCore(scheduler, logQueue, logobj, TASK_PATH)
...
def get_job(name_id):
    ...
    with open(path, 'r') as rh:
        ...
        return json.dumps({'err': 0, 'load_info': load_info, 'status': status,
                           'path': path})
...
def add_job():
    ...
    if git_address is None or name_id is None:
